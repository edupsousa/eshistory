...
function SqlGenerator() {
    ...
    this.update = function(table, values, where) {
        var sqlQuery = "UPDATE " + parseFieldName(table) + " SET ";
        var updateValues = [];
        for (var fieldName in values) {
            updateValues.push(parseFieldName(fieldName) + "=" + parseValueType(values[fieldName]));
        }
        sqlQuery += updateValues.join(",");
...
function MySqlExporter(outputPath) {
    ...
    function getExportCommitsSQL(commits) {
        ...
        for (var i = 0; i < commits.length; i++) {
            ...
            commitsToInsert.push({
                ...
                author: authorQuery,
                extracted: (commit.extractFiles ? 1 : 0)
            });
        }
    ...
    this.exportReferences = function(references) {
        var sql = getExportReferencesSQL(references);
        return appendToOutput(sql);
    };

    function getExportReferencesSQL(references) {
        var refsToInsert = [];

        for (var i = 0; i < references.length; i++) {
            var reference = references[i];

            var type = (reference.isTag ? "tag" : (reference.isRemote ? "remote" : "branch"));
            var target = new self.sql.Expression(
                self.sql.select(["id"], "commit", "project = @project_id AND commit_oid = '" + reference.target + "'")
            );
        ...
        return self.sql.insertMultiple("reference", refsToInsert) + "\n";
    }
    ...
    function getExportFileMetricsSQL(filesMetrics) {
        ...
        for (var i = 0; i < filesMetrics.length; i++) {
            ...
            if (!fileMetrics.error) {
                sqlQuery += self.sql.set("entry_id", selectIdFromFileEntry(fileMetrics.id)) + ";\n";
                sqlQuery += getInsertFileMetricsSQL(fileMetrics.data);
            } else {
                sqlQuery += self.sql.update("file_entry",
                    {
                        has_error: 1,
                        error_reason: fileMetrics.reason
                    },
                    "project=@project_id AND entry_oid='" + fileMetrics.id + "'") + ";\n";
            }
        }
        return sqlQuery;
    }
        ...
        var sqlQuery = self.sql.insert("file_metrics",
            {
                ...
                cyclomatic_density: (metrics.cyclomaticDensity === null ? 0 : metrics.cyclomaticDensity),
                functions: metrics.functionCount,
                dependencies: metrics.dependencyCount,
                maintainability: metrics.maintainability,
                hs_operators_total: metrics.hsOperatorsTotal,
                hs_operators_distinct: metrics.hsOperatorsDistinct,
                hs_operands_total: metrics.hsOperandsTotal,
                hs_operands_distinct: metrics.hsOperandsDistinct,
                hs_vocabulary: metrics.hsVocabulary,
                hs_length: metrics.hsLength,
                hs_difficulty: metrics.hsDifficulty,
                hs_volume: metrics.hsVolume,
                hs_effort: metrics.hsEffort,
                hs_bugs: metrics.hsBugs,
                hs_time: metrics.hsTime
            }) + ";\n";
