...
ProjectMetricsCommand.extractProjectMetrics = function () {
    ...
    return this.createExtractor()
        .then(function(extractor) {
            ...
            self.verboseLog("Extracting commits and authors.");
            return extractor.getCommitsAndAuthors()
                .then(function(result) {
                    self.exportAuthors(result.authors);
                    self.exportCommits(result.commits);

                    self.verboseLog("Extracting files.");
                    return extractor.getFilesFromCommits(result.commits);
                })
                .then(function(result) {
                    ...
                    return extractor.getMetricsForEntries(result.entries);
                })
...
ProjectMetricsCommand.createExtractor = function () {
    ...
    return MetricsExtractor.getExtractorForRepository(this.repositoryPath);
};
