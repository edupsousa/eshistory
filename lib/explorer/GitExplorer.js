...
function createWorkers(numbeOfInstances) {
    var workers = [];
    for (var i = 0; i < numbeOfInstances; i++) {
        workers[i] = cp.fork(__dirname + '/GitExplorerWorker.js');
    }
    return workers;
}
...
function configureWorkers(workers, repositoryPath) {
    ...
    var configListener = function(message) {
        ...
        if (answersCount === workers.length) {
            for (var i = 0; i < workers.length; i++) {
                workers[i].removeListener('message', configListener);
            }
    ...
    for (var i = 0; i < workers.length; i++) {
        workers[i].on('message', configListener);
        workers[i].send({
            what: 'configure',
            value: {
...
function sendMessagesToWorkers(workers, messagesToSend, responseCallback) {
    var deferred = Q.defer();
    var responsesExpected = messagesToSend.length;
    ...
    for (var i = 0; i < workers.length; i++) {
        workers[i].on('message', responseListener.bind(null, workers[i]));
        if (messagesToSend.length > 0)
            workers[i].send(messagesToSend.pop());
    }
...
GitExplorer.prototype.listFiles = function(commits) {
    ...
    configureWorkers(workers, this.gitRepository.path())
        .then(function() {
            return sendMessagesToWorkers(workers, messagesToSend, messageReceiveCallback)
        }).then(function() {
            killWorkers(workers);
            deferred.resolve({
                paths: paths,
