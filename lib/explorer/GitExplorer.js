...
GitExplorer.prototype.getCommits = function(branch) {
    var self = this;
    var commits = [];
    ...
    if (branch) {
        promise = promise.then(function() {
            return self.gitRepository.getBranchCommit(branch);
        }).then(function(commit) {
            return self.createHistoryWalker(commit.id());
        });
    } else {
        promise = promise.then(function() {
            return self.createHistoryWalker();
        });
    }

    promise = promise.then(function(walkHistory) {
