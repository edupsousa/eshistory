...
GitExplorer.prototype.createHistoryWalker = function(commitOid, idFilterList) {
    ...
    var recursiveWalker = function(walkFn) {
        return revWalk.next().then(function(id) {
            if (!id) return;
            if (idFilterList.length === 0 || _.includes(idFilterList, id.toString()))
                walkFn(id);
...
GitExplorer.prototype.getCommitsAndAuthors = function(filter) {
    ...
    if (filter.onlyTags) {
        promise = promise.then(function() {
            return self.gitRepository.getReferences(NodeGit.Reference.TYPE.OID)
                .then(function(refList) {
                    console.log(refList.length);
                    for (var i = 0 ; i < refList.length; i++) {
                        var ref = refList[i];
                        if (ref.isTag()) {
                            var id = ref.target().toString();
                            commitsToFilter.push(id);
                        }
                    }
                });
        });
    ...
    promise = promise.then(function() {
        ...
        console.log(commitsToFilter);
        return self.createHistoryWalker(commit.id(), commitsToFilter);
