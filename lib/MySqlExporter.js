...
function MySqlExporter() {
    ...
    this.writeAuthors = function(authors) {
        ...
        for (var i = 0; i < authors.length; i++) {
            sqlQuery += this.writeAuthor(authors[i]);
        }
        return sqlQuery;
    };
    ...
    this.writeCommit = function(commit) {
        var authorQuery = new this.sql.Expression(this.sql.select(
            ["id"],
            "author",
            "`name`='" + this.sql.escapeString(commit.author.name) + "' AND `email`='" + this.sql.escapeString(commit.author.email) + "'"));
    ...
    this.writeCommits = function(commits) {
        ...
        for (var i = 0; i < commits.length; i++)
            sqlQuery += this.writeCommit(commits[i]);
        return sqlQuery;
    };
    ...
    this.writeFileEntries = function(entries) {
        ...
        for (var i = 0; i < entries.length; i++) {
            sqlQuery += this.writeFileEntry(entries[i]);
        }
        return sqlQuery;
    };
    ...
    this.writePaths = function(paths) {
        ...
        for (var i = 0; i < paths.length; i++) {
            sqlQuery += this.writePath(paths[i]);
        }
        return sqlQuery;
    };
    ...
    this.writeCommitFile = function(commitOid, entryFileOid, path) {
        return this.sql.insert("commit_file",
            {
                commit: new this.sql.Expression(selectIdFromCommit(commitOid)),
                file_entry: new this.sql.Expression(selectIdFromFileEntry(entryFileOid)),
                path: new this.sql.Expression(selectIdFromPath(path))
            }) + ";\n";
    };
    ...
    this.writeCommitFiles = function(commitOid, files) {
        ...
        for (var i = 0; i < files.length; i++) {
            var file = files[i];
            sqlQuery += this.writeCommitFile(commitOid, file.id, file.path);
        }
        return sqlQuery;
    };
    ...
    this.writeFilesMetrics = function(filesMetrics) {
        ...
        for (var i = 0; i < filesMetrics.length; i++) {
            ...
            if (!fileMetrics.error)
                sqlQuery += this.writeFileMetrics(fileMetrics.id, fileMetrics.data);
        }
        return sqlQuery;
    };
    ...
    this.writeFileMetrics = function(fileOid, metrics) {
        var sqlQuery = this.sql.insert("file_metrics",
            {
                file_entry: new this.sql.Expression(selectIdFromFileEntry(fileOid)),
                loc: metrics.loc,
